...
class TestVPCProviderMixin(unittest.TestCase):
    ...
    def test_resolve_vpc_from_ssm_imports(self, mock_from_vpc_attrs):
        ...
        # Create mock configs with SSM imports
        config = MagicMock()
        config.ssm = {"imports": {"vpc_id": "vpc-12345"}}
        workload = MagicMock()
    ...
    def test_resolve_vpc_from_ssm_with_subnets(self, mock_from_vpc_attrs):
        ...
        # Create mock configs with SSM imports including subnets
        config = MagicMock()
        config.ssm = {
            "imports": {
                "vpc_id": "vpc-12345",
    ...
    def test_resolve_vpc_from_config(self, mock_from_lookup):
        ...
        mock_from_lookup.assert_called_once_with(
            self.stack, "TestStack-VPC", vpc_id="vpc-config-123"
        )
    ...
    def test_resolve_vpc_from_workload(self, mock_from_lookup):
        ...
        self.assertEqual(result, mock_vpc)
        mock_from_lookup.assert_called_once_with(
            self.stack, "TestStack-VPC", vpc_id="vpc-workload-456"
        )
    ...
    def test_resolve_vpc_custom_availability_zones(self, mock_from_vpc_attrs):
        ...
        # Resolve VPC
        result = self.stack.resolve_vpc(
            config, self.deployment_config, workload, custom_azs
        )
    ...
    def test_get_default_azs_for_region(self):
        """Test region-specific AZ mapping"""
        # Test major regions
        self.assertEqual(
            self.stack._get_default_azs_for_region("us-east-1"),
            ["us-east-1a", "us-east-1b"]
        )
        self.assertEqual(
            self.stack._get_default_azs_for_region("us-west-2"),
            ["us-west-2a", "us-west-2b"]
        )
        self.assertEqual(
            self.stack._get_default_azs_for_region("eu-west-1"),
            ["eu-west-1a", "eu-west-1b"]
        )
        
        # Test unknown region falls back to us-east-1
        self.assertEqual(
            self.stack._get_default_azs_for_region("unknown-region"),
            ["us-east-1a", "us-east-1b"]
        )

    @patch("cdk_factory.interfaces.vpc_provider_mixin.ec2.Vpc.from_vpc_attributes")
    def test_resolve_vpc_uses_region_specific_defaults(self, mock_from_vpc_attrs):
        """Test that VPC resolution uses region-specific default AZs"""
        # Mock VPC creation
        mock_vpc = MagicMock()
        mock_from_vpc_attrs.return_value = mock_vpc

        # Create deployment config with different region
        deployment_config_west = DeploymentConfig(
            workload=self.workload_config.dictionary,
            deployment={"name": "test-deployment", "environment": "prod", "region": "us-west-2"},
        )
        ...
        # Resolve VPC with us-west-2 deployment
        result = self.stack.resolve_vpc(config, deployment_config_west, workload)

        # Verify VPC was created with us-west-2 AZs
        call_args = mock_from_vpc_attrs.call_args
        vpc_attrs = call_args[1]  # kwargs
        self.assertEqual(vpc_attrs["availability_zones"], ["us-west-2a", "us-west-2b"])
