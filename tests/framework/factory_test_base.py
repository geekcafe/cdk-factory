...
class FactoryTestBase:
    ...
    def setUp(self):
        ...
        self.app = cdk.App(
            context={
                "cdk-factory": {"testing": True, "environment": "test"},
                "ENVIRONMENT": "test",
        ...
        return WorkloadConfig(
            {
                ...
                "owner": "test-team",
                "description": "Test workload for CDK Factory testing",
                "devops": {"aws_account": "123456789012", "region": "us-east-1"},
            }
        )

    def _create_test_stack_config(
    ...
    def synthesize_stack(
        self, stack_class, config_override: Dict[str, Any] = None
    ) -> Dict[str, Any]:
        """
    ...
    def synthesize_stack_with_context(
        self, stack_class, context: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
    ...
    def validate_ssm_integration(
        self,
        template: Dict[str, Any],
        expected_imports: List[str] = None,
        expected_exports: List[str] = None,
    ) -> Dict[str, Any]:
        """
    ...
    def create_test_config_with_ssm(
        self,
        module_name: str,
    ...
    def assert_template_valid(
        ...
        if expected_resources is not None:
            ...
            ), f"Expected {expected_resources} resources, got {len(resources)}"

        # Check SSM parameters
        ssm_params = self.extract_ssm_parameters(template)
        if expected_ssm_params is not None:
            assert (
                len(ssm_params) == expected_ssm_params
            ), f"Expected {expected_ssm_params} SSM parameters, got {len(ssm_params)}"
