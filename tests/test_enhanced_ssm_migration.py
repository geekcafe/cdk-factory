...
def test_enhanced_base_config():
    ...
    enhanced_config = EnhancedBaseConfig(
        config, resource_type="vpc", resource_name="test-vpc"
    )
    ...
    expected_path = "/test-org/dev/vpc/test-vpc/vpc-id"
...
def test_vpc_config_migration():
    ...
    config = {
        "name": "test-vpc",
        "cidr": "10.0.0.0/16",
        "ssm": {"enabled": True, "organization": "test-org", "environment": "dev"},
    }
...
def test_ssm_mixin():
    ...
    config = {
        "ssm": {
            ...
            "auto_export": True,
            "auto_import": True,
            "exports": {"test_param": "/test/env/resource/test-param"},
            "imports": {"imported_param": "/test/env/resource/path"},
        }
    }

    enhanced_config = EnhancedBaseConfig(
        config, resource_type="test", resource_name="test-resource"
    )
    ...
    # Test setup
    mixin.setup_ssm_integration(
        mock_scope, enhanced_config, resource_type="test", resource_name="test-resource"
    )
    assert hasattr(mixin, "resource_type")
    assert mixin.resource_type == "test"
...
def test_environment_variable_support():
    ...
    vpc_id_path = enhanced_config.get_parameter_path("vpc_id")
    expected_path = "/test-org/production/vpc/test-vpc/vpc-id"
    assert vpc_id_path == expected_path, f"Expected {expected_path}, got {vpc_id_path}"
...
def test_custom_patterns():
    ...
    enhanced_config = EnhancedBaseConfig(
        config, resource_type="api_gateway", resource_name="cdk-factory-api-gw"
    )

    # Test custom pattern
    api_id_path = enhanced_config.get_parameter_path("api_gateway_id")
    expected_path = "/acme-corp/prod/cdk-factory-api-gw-api-gateway-id"
