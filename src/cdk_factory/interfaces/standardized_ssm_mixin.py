...
class StandardizedSsmMixin:
    ...
    def export_resource_to_ssm(
        ...
        for key, export_path in config.ssm_exports.items():
            ...
            if key in resource_values:
                ...
                logger.warning(
                    f"SSM export configured for '{key}' but no value found in resource_values"
                )
    ...
    def setup_ssm_integration(
        ...
        logger.info(
            f"Setup standardized SSM integration for {resource_type}/{resource_name}"
        )
    ...
    def process_ssm_imports(self) -> None:
        ...
        if not imports:
            logger.info(
                f"No SSM imports configured for {self.resource_type}/{self.resource_name}"
            )
            return

        logger.info(
            f"Processing {len(imports)} SSM imports for {self.resource_type}/{self.resource_name}"
        )
    ...
    def export_ssm_parameters(self, resource_values: Dict[str, Any]) -> Dict[str, str]:
        ...
        if not exports:
            logger.info(
                f"No SSM exports configured for {self.resource_type}/{self.resource_name}"
            )
            return {}

        logger.info(
            f"Exporting {len(exports)} SSM parameters for {self.resource_type}/{self.resource_name}"
        )
        ...
        for export_key, export_path in exports.items():
            if export_key not in resource_values:
                logger.warning(
                    f"Export key '{export_key}' not found in resource values"
                )
                continue
    ...
    def resolve_ssm_value(self, scope: Construct, value: str, unique_id: str) -> str:
        ...
        if "{{ssm" in value and not value.startswith("{{ssm"):
            ...
            if match:
                ...
                ssm_value = self.resolve_ssm_value(
                    scope, ssm_ref, f"{unique_id}-embedded"
                )
                # Replace the SSM reference with resolved value in the original string
                resolved_value = value.replace(ssm_ref, ssm_value)
                logger.info(
                    f"Resolved embedded SSM in string: {value} -> {resolved_value}"
                )
        ...
        for pattern in ssm_patterns:
            if value.startswith(pattern["prefix"]) and value.endswith("}}"):
                ...
                logger.info(
                    f"Resolved SSM {pattern['type']} parameter: {ssm_param_path}"
                )
                return resolved_value
    ...
    def _resolve_ssm_import(
        self, import_value: Union[str, List[str]], import_key: str
    ) -> Union[str, List[str]]:
        """
        ...
        if isinstance(import_value, list):
            ...
            for i, value in enumerate(import_value):
                resolved_item = self._resolve_single_ssm_import(
                    value, f"{import_key}[{i}]"
                )
    ...
    def _resolve_single_ssm_import(self, ssm_path: str, context: str) -> str:
        ...
        construct_id = (
            f"import-{context.replace('.', '-').replace('[', '-').replace(']', '-')}"
        )
    ...
    def _create_ssm_parameter(
        self, export_key: str, export_path: str, value: Any
    ) -> ssm.StringParameter:
        """
    ...
    def get_subnet_ids(self, config) -> List[str]:
        ...
        if "subnet_ids" in ssm_imports:
            ...
            if isinstance(subnet_ids, str):
                ...
                parsed_ids = [
                    sid.strip() for sid in subnet_ids.split(",") if sid.strip()
                ]
                return parsed_ids
...
class SsmStandardValidator:
    ...
    def _validate_import(self, key: str, value) -> List[str]:
        ...
        Keys like 'workload', 'environment', and 'organization' are name
        components used to *build* SSM paths — they are not paths themselves
