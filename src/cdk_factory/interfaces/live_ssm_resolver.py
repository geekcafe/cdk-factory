...
from boto3_assist import SSMAssist
...
class LiveSsmResolver:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.mode = self.config.get("mode", "fallback")  # "always", "fallback", "disabled"
    ...
    def _init_ssm_client(self):
        ...
        try:
            ...
            self._ssm_client = SSMAssist(
                profile_name=profile,
                region_name=region
            )
            logger.info(f"Initialized live SSM resolver with profile: {profile}, region: {region}")
        except Exception as e:
            logger.warning(f"Failed to initialize SSM client: {e}")
            self.enabled = False
    
    def resolve_parameter(self, parameter_path: str, fallback_value: Optional[str] = None) -> Optional[str]:
        """
    ...
    def resolve_parameters_batch(self, parameter_paths: list[str]) -> Dict[str, Optional[str]]:
        """
    ...
    def _fetch_parameters_batch(self, parameter_paths: list[str]) -> Dict[str, Optional[str]]:
