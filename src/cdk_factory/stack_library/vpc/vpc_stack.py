...
class VpcStack(IStack, StandardizedSsmMixin):
    ...
    def _build(
        ...
        if imported_resources:
            logger.info(
                f"Imported resources from SSM: {list(imported_resources.keys())}"
            )
    ...
    def _create_vpc(self, vpc_name: str, stable_vpc_id: str) -> ec2.Vpc:
        ...
        vpc_props = {
            ...
            "restrict_default_security_group": self.vpc_config.get(
        ...
        if namespace and self.stack_config.ssm_auto_export:
            ...
            for export_key, export_value in resource_values.items():
                ...
                self.export_ssm_parameter(
                    ...
                    description=f"VPC {export_key}",
                )
            logger.info(f"Auto-exported {len(resource_values)} VPC parameters to SSM")
        else:
    ...
    def auto_export_resources(
        self, resource_values: Dict[str, Any], context: Dict[str, Any] = None
    ) -> Dict[str, str]:
