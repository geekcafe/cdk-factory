...
class EcsServiceStack(IStack, VPCProviderMixin, StandardizedSsmMixin):
    ...
    def __init__(self, scope: Construct, id: str, **kwargs) -> None:
        ...
        self.task_definition: (
            Optional[ecs.FargateTaskDefinition] | Optional[ecs.EcsTaskDefinition]
        ) = None
    ...
    def _build(
        ...
        self.ecs_config = EcsServiceConfig(
            ...
            or stack_config.dictionary.get("ecs", {})
        )
        ...
        if self.ecs_config.service_name:
            ...
            logger.info(
                "Service name not specified - CloudFormation will auto-generate for safe replacement"
            )
        ...
        self._create_task_definition(
            service_name or deployment.build_resource_name(self.ecs_config.name)
        )
        ...
        for param_key, param_value in ssm_imports.items():
            try:
                ...
                if isinstance(param_value, list):
                    ...
                    logger.info(
                        f"Imported SSM parameter list: {param_key} with {len(imported_list)} items"
                    )
                else:
                    ...
                    logger.info(
                        f"Imported SSM parameter: {param_key} from {param_path}"
                    )
    ...
    def _create_or_load_cluster(self) -> None:
        ...
        if cluster_name:
            ...
            try:
                ...
                logger.warning(
                    f"Could not load cluster {cluster_name}, creating new one: {e}"
                )
                self._create_new_cluster(cluster_name)
        else:
            # Create a new cluster with auto-generated name
            cluster_name = (
                f"{self.deployment.workload_name}-{self.deployment.environment}-cluster"
            )
            self._create_new_cluster(cluster_name)
    ...
    def _create_task_definition(self, service_name: str) -> None:
        ...
        if self.ecs_config.launch_type == "EC2":
            ...
            self.task_definition = ecs.Ec2TaskDefinition(
                ...
                network_mode=(
                    ecs.NetworkMode(network_mode.upper())
                    if network_mode
                    else ecs.NetworkMode.BRIDGE
                ),
    ...
    def _add_volumes_to_task(self) -> None:
        ...
        for volume_config in volumes:
            ...
            if "host" in volume_config:
                ...
                if self.ecs_config.launch_type != "EC2":
                    logger.warning(
                        f"Host volumes are only supported for EC2 launch type, skipping volume {volume_name}"
                    )
                    continue
                ...
                if not source_path:
                    logger.warning(
                        f"Host source_path is required for volume {volume_name}, skipping"
                    )
                    continue

                # Add host volume to task definition
                self.task_definition.add_volume(
                    name=volume_name, host=ecs.Host(source_path=source_path)
                )
                ...
                if not file_system_id:
                    logger.warning(
                        f"EFS file_system_id is required for volume {volume_name}, skipping"
                    )
                    continue
                ...
                efs_volume_config = ecs.EfsVolumeConfiguration(
                    ...
                    transit_encryption=(
                        "ENABLED"
                        if efs_config.get("transit_encryption", True)
                        else "DISABLED"
                    ),
                    authorization_config=(
                        ecs.AuthorizationConfig(
                            access_point_id=efs_config.get("access_point_id")
                        )
                        if efs_config.get("access_point_id")
                        else None
                    ),
                )

                self.task_definition.add_volume(
                    name=volume_name, efs_volume_configuration=efs_volume_config
                )

                logger.info(f"Added EFS volume: {volume_name} -> {file_system_id}")
            else:
                logger.warning(
                    f"Volume {volume_name} must specify either 'host' or 'efs' configuration"
                )
    ...
    def _add_containers_to_task(self) -> None:
        ...
        for idx, container_config in enumerate(container_definitions):
            ...
            if health_check_config:
                health_check = ecs.HealthCheck(
                    ...
                    interval=cdk.Duration.seconds(
                        health_check_config.get("interval", 30)
                    ),
                    timeout=cdk.Duration.seconds(health_check_config.get("timeout", 5)),
                    retries=health_check_config.get("retries", 3),
                    start_period=cdk.Duration.seconds(
                        health_check_config.get("start_period", 0)
                    ),
                )
            ...
            container = self.task_definition.add_container(
                ...
                environment=self._load_environment_variables(
            ...
            for mount_point in mount_points:
                ...
                if not source_volume or not container_path:
                    logger.warning(
                        f"Mount point requires source_volume and container_path, skipping"
                    )
                    continue
                ...
                logger.info(
                    f"Added mount point: {source_volume} -> {container_path} (read_only={read_only})"
                )
    ...
    def _build_linux_parameters(self, config: Dict[str, Any]) -> ecs.LinuxParameters:
        ...
        for device in devices:
            ...
            logger.info(
                f"Added device mapping: {host_path} -> {container_path} with permissions {permissions}"
            )

        return linux_params

    def _load_environment_variables(
        self, environment_variables_config: Dict[str, str]
    ) -> Dict[str, str]:
        ...
        if self.ecs_config.launch_type == "EC2":
            service_kwargs = {
                ...
                "health_check_grace_period": (
                    cdk.Duration.seconds(self.ecs_config.health_check_grace_period)
                    if self.ecs_config.target_group_arns
                    else None
                ),
                "circuit_breaker": (
                    ecs.DeploymentCircuitBreaker(
                        enable=self.ecs_config.deployment_circuit_breaker.get(
                            "enable", True
                        ),
                        rollback=self.ecs_config.deployment_circuit_breaker.get(
                            "rollback", True
                        ),
                    )
                    if self.ecs_config.deployment_circuit_breaker
                    else None
                ),
                "placement_strategies": self._get_placement_strategies(),
                "placement_constraints": self._get_placement_constraints(),
                "max_healthy_percent": self.ecs_config.deployment_configuration.get(
                    "maximum_percent"
                ),
                "min_healthy_percent": self.ecs_config.deployment_configuration.get(
            ...
            if capacity_provider_strategies:
                service_kwargs["capacity_provider_strategies"] = (
                    capacity_provider_strategies
                )

            self.service = ecs.Ec2Service(self, "Service", **service_kwargs)
        else:
            # Fargate service
            fargate_kwargs = {
                "cluster": self.cluster,
                ...
                "vpc_subnets": (
                    ec2.SubnetSelection(subnets=subnets) if subnets else None
                ),
            ...
            self.service = ecs.FargateService(self, "Service", **fargate_kwargs)
    ...
    def _build_capacity_provider_strategy(
        self,
    ) -> Optional[List[ecs.CapacityProviderStrategy]]:
        ...
        for strategy in strategy_config:
            ...
            resolved_cp_name = self.resolve_ssm_value(
                scope=self, value=cp_name, unique_id=f"cp-{cp_name}"
            )
        ...
        for param_key, param_name in ssm_imports.items():
            if "target_group" in param_key.lower() or "tg" in param_key.lower():
                try:
                    ...
                    logger.warning(
                        f"Could not load target group from SSM {param_name}: {e}"
                    )
