...
class RdsConfig(EnhancedBaseConfig):
    ...
    def __init__(self, config: dict, deployment) -> None:
        super().__init__(
            config or {},
            resource_type="rds",
            resource_name=config.get("name", "rds") if config else "rds",
        )
    ...
    def _sanitize_instance_identifier(self, identifier: str) -> str:
        ...
        if notes:
            logger.info(
                f"Sanitized instance identifier from '{identifier}' to '{sanitized}': {', '.join(notes)}"
            )

        return sanitized

    def _sanitize_instance_identifier_impl(
        self, identifier: str
    ) -> Tuple[str, List[str]]:
        """
        ...
        if not s:
            raise ValueError(
                f"Instance identifier '{identifier}' contains no valid characters"
            )
    ...
    def master_username(self) -> str:
        ...
        if not raw_username:
            raise ValueError(
                "No master username found. Please add the master username to the config."
            )
    ...
    def cloudwatch_logs_exports(self) -> List[str]:
        ...
        # Default to empty list for unknown engines (safer than guessing)
            ...
            logger.warning(
                f"Unknown engine '{engine}', disabling CloudWatch logs exports by default"
            )
            return []
    ...
    def _sanitize_database_name(self, name: str) -> str:
        ...
        if notes:
            logger.info(
                f"Sanitized database name from '{name}' to '{sanitized}': {', '.join(notes)}"
            )
    ...
    def _sanitize_db_name_impl(self, engine: str, name: str) -> Tuple[str, List[str]]:
        ...
        if not s:
            raise ValueError(
                f"Database name '{name}' contains no valid characters after sanitization"
            )
    ...
    def _sanitize_username(self, username: str) -> str:
        ...
        if notes:
            logger.info(
                f"Sanitized username from '{username}' to '{sanitized}': {', '.join(notes)}"
            )
    ...
    def _sanitize_master_username_impl(self, username: str) -> Tuple[str, List[str]]:
        ...
        if not s:
            raise ValueError(
                f"Username '{username}' contains no valid characters after sanitization"
            )
